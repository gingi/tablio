...
interface SeatContextMenuProps {
  ...
  currentGuest?: Guest;
  allGuests: Guest[];
  allTables: any[]; // We'll need this to show table names
  onClose: () => void;
...
export function SeatContextMenu({
  ...
  const categories = Array.from(new Set(allGuests.map(g => g.category).filter(Boolean))) as string[];
  ...
  return (
    ...
    >
      <Card 
        ...
        style={{
          left: Math.max(10, Math.min(position.x - 160, Math.max(window.innerWidth - 330, 10))),
          top: Math.max(10, Math.min(position.y - 100, Math.max(window.innerHeight - 500, 10)))
        }}
      >
        <div className="p-4">
              ...
              <div className="space-y-1">
                {filteredGuests.length === 0 ? (
                  <div className="text-center text-muted-foreground py-4 text-sm">
                    {searchTerm || categoryFilter !== 'all' ? 'No guests found' : 'No guests available'}
                  </div>
                ) : (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-1 mt-1 flex-wrap">
                          ...
                          <Badge 
                            variant={guest.tableId ? "default" : "outline"} 
                            className="text-xs"
                          >
