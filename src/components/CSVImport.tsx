...
export function CSVImport({ onImport }: CSVImportProps) {
  ...
  const parseCSV = (csvText: string): Guest[] => {
    ...
    const dietaryIndex = headers.findIndex(h => h.includes('dietary') || h.includes('diet') || h.includes('allergy'));
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length > nameIndex && values[nameIndex]) {
        const guest: Guest = {
          ...
          dietary: dietaryIndex !== -1 && values[dietaryIndex] ? values[dietaryIndex] : undefined,
        };
        guests.push(guest);
      }
    }
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="text-sm text-muted-foreground mb-2 block">Import Guest List</label>
        <Card className="p-4">
          <div className="space-y-3">
