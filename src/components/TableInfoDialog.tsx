...
export function TableInfoDialog({ 
  ...
  const getGuestCardCategory = (guest: Guest) => {
    switch (guest.category) {
      ...
      case 'Friends': return 'bg-green-100 text-green-800 border-green-300';
      case 'Colleagues': return 'bg-purple-100 text-purple-800 border-purple-300';
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
                ...
                table.guests.map((guest, index) => (
                  <Card 
                    ...
                    className={`p-3 cursor-grab hover:shadow-md transition-all ${
                      draggedGuest?.id === guest.id ? 'opacity-50 scale-95' : ''
                    }`}
                    draggable
          ...
          <div className="text-xs text-muted-foreground">
            <p>💡 <strong>Tip:</strong> Drag guest cards to other tables to reassign them</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
