...
const SAMPLE_NAMES = [
  'Alice Johnson', 'Bob Smith', 'Carol Davis', 'David Wilson', 'Emma Brown', 'Frank Miller', 'Grace Lee', 'Henry Taylor',
  'Isabella Garcia', 'Jack Anderson', 'Katherine Martinez', 'Liam Rodriguez', 'Mia Hernandez', 'Noah Lopez', 'Olivia Gonzalez',
  'Parker Wilson', 'Quinn Perez', 'Rachel Moore', 'Samuel Taylor', 'Taylor Thomas', 'Uma Jackson', 'Victor White', 'Wendy Harris',
  'Xavier Martin', 'Yara Thompson', 'Zachary Garcia', 'Ava Clark', 'Benjamin Lewis', 'Charlotte Robinson', 'Daniel Walker',
  'Emily Hall', 'Felix Young', 'Gabriela Allen', 'Harrison King', 'Iris Wright', 'Julian Scott', 'Kaitlyn Green', 'Lucas Adams',
  'Madison Baker', 'Nathan Nelson', 'Ophelia Carter', 'Preston Mitchell', 'Quincy Perez', 'Ruby Roberts', 'Sebastian Turner',
  'Tessa Phillips', 'Ulysses Campbell', 'Violet Parker', 'William Evans', 'Xiomara Edwards', 'Yasmin Collins'
];
...
const INITIAL_GUESTS: Guest[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  dietary: Math.random() > 0.8 ? ['Vegetarian', 'Vegan', 'Gluten-Free'][Math.floor(Math.random() * 3)] : undefined
}));
...
export function TablePlanner() {
  ...
  const addToHistory = useCallback((action: string, prevGuests: Guest[], prevTables: TableData[]) => {
    setHistory(prev => {
  ...
  // Add new guest
  const addGuest = useCallback((name: string, category?: string, dietary?: string, tableId?: string) => {
    if (!name.trim()) {
    ...
    if (tableId) {
      setTables(prev => prev.map(table => {
        if (table.id === tableId) {
          const occupiedSeats = new Set(table.guests.map(g => g.seatNumber).filter(s => s !== undefined));
          ...
          return {
            ...table,
            guests: [...table.guests, guestWithSeat].sort((a, b) => (a.seatNumber || 0) - (b.seatNumber || 0))
          };
        }
        return table;
      }));
  ...
  const assignGuestToTableInternal = useCallback((guestId: string, tableId: string, seatNumber?: number) => {
    const guest = guests.find(g => g.id === guestId);
    if (!guest) return;
    ...
    if (seatNumber !== undefined) {
      ...
      if (targetTable) {
        displacedGuest = targetTable.guests.find(g => g.seatNumber === seatNumber) || null;
      }
    }
    ...
    setTables(prev => prev.map(table => {
      if (table.id === tableId) {
        ...
        if (seatNumber !== undefined) {
          ...
          updatedGuests = updatedGuests.filter(g => g.seatNumber !== seatNumber);
          ...
          const occupiedSeats = new Set(updatedGuests.map(g => g.seatNumber).filter(s => s !== undefined));
          let nextAvailableSeat = 0;
          while (occupiedSeats.has(nextAvailableSeat) && nextAvailableSeat < 12) {
            nextAvailableSeat++;
          }
          ...
          if (nextAvailableSeat < 12) {
            updatedGuests.push({ ...guest, tableId, seatNumber: nextAvailableSeat });
            updatedGuests.sort((a, b) => (a.seatNumber || 0) - (b.seatNumber || 0));
          }
        }
  ...
  const assignGuestToTable = useCallback((guestId: string, tableId: string, seatNumber?: number) => {
  ...
  const assignMultipleGuestsToTable = useCallback((guestIds: string[], tableId: string) => {
    const table = tables.find(t => t.id === tableId);
    if (!table) return;
  ...
  const addTable = useCallback((seats: number) => {
    ...
    const newTable: TableData = {
      ...
      seats: 12, // Always 12 seats for oval tables
      x: 200 + (tables.length % 4) * 250,
      y: 150 + Math.floor(tables.length / 4) * 250,
      guests: []
    };
    setTables(prev => [...prev, newTable]);
  }, [tables.length, guests, tables, addToHistory]);
  ...
  const importGuests = useCallback((newGuests: Guest[]) => {
    addToHistory('Import guests', guests, tables);
    
    // Remove any existing sample guests and add imported guests
    setGuests(prev => {
      const existingImported = prev.filter(g => g.id.startsWith('imported-'));
      const nonSample = prev.filter(g => !g.id.startsWith('guest-') && !g.id.startsWith('imported-'));
      return [...nonSample, ...newGuests];
    });
  }, [guests, tables, addToHistory]);

  const importLayout = useCallback((data: { guests: Guest[]; tables: TableData[] }) => {
  ...
  const autoLayoutTables = useCallback(() => {
    ...
    const tableWidth = 320; // Table width (240px) + generous margin
    const tableHeight = 240; // Table height (160px) + generous margin
    const minX = 200; // Start further from edges to account for centering
    const minY = 150;
    
    // Calculate columns and rows for the best fit
    let cols, rows;
    if (tables.length <= 3) {
      ...
      rows = Math.ceil(tables.length / cols);
    }
        ...
        {tables.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <Card className="p-8 text-center">
              <h3>No tables yet</h3>
              <p className="text-muted-foreground">Add your first table using the controls in the sidebar</p>
            </Card>
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex h-screen bg-background">
        ...
        >
          <div className={`flex items-center justify-between gap-2 p-4 border-b border-border flex-shrink-0 ${sidebarCollapsed ? 'p-2' : ''}`}>
            {sidebarCollapsed ? (
          ...
          <div className={`flex-1 overflow-y-auto ${sidebarCollapsed ? 'opacity-0 pointer-events-none select-none' : 'opacity-100'} transition-opacity duration-200`}>        
            {!sidebarCollapsed && (
              <div className="flex flex-col">
                ...
                <GuestList 
                  ...
                  onAssignGuest={assignGuestToTable}
                  onAssignMultipleGuests={assignMultipleGuestsToTable}
