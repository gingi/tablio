...
import { Plus, Users, Download, Upload, Save, FolderOpen, Grid3X3, Undo, RotateCcw } from 'lucide-react';
...
export function Controls({ onAddTable, onImportGuests, onImportLayout, onAutoLayout, onUndo, onResetAssignments, tables, guests, canUndo }: ControlsProps) {
  ...
  return (
    <div className="p-4">
      ...
      <div className="space-y-3 mt-3">
        <div>
          <label className="text-sm text-muted-foreground mb-2 block">Quick Actions</label>
        ...
        <div>
          <label className="text-sm text-muted-foreground mb-2 block">Add New Table</label>
          <Button
            variant="outline"
            size="sm"
        ...
        <div>
          <label className="text-sm text-muted-foreground mb-2 block">Import Layout</label>
          <input
            ref={fileInputRef}
            type="file"
            accept=".json"
        ...
        <div>
          <label className="text-sm text-muted-foreground mb-2 block">Current Tables</label>
          <div className="space-y-1 max-h-32 overflow-y-auto">
            {tables.length === 0 ? (
              <div className="text-xs text-muted-foreground italic">No tables added yet</div>
            ) : (
              tables.map(table => (
